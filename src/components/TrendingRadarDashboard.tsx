...
export const TrendingRadarDashboard: React.FC<TrendingRadarDashboardProps> = ({ repos }) => {
  ...
  const rankBadges = [
    { label: '👑 #1 APEX BREAKOUT', color: 'from-amber-400 to-orange-500', text: 'text-black' },
    { label: '⚡ #2 VIRAL SURGE', color: 'from-cyan-400 to-blue-500', text: 'text-black' },
    { label: '🚀 #3 HYPER RISING', color: 'from-orange-400 to-amber-500', text: 'text-black' },
    { label: '💎 #4 COMMUNITY GEM', color: 'from-emerald-400 to-teal-500', text: 'text-black' },
    { label: '🌐 #5 RADAR SIGNAL', color: 'from-violet-400 to-purple-500', text: 'text-white' },
  ];
  ...
  return (
    <section className="relative w-full max-w-5xl mx-auto px-4 sm:px-6 pt-4 pb-8 font-sans">
      ...
      <div className="text-center mb-8 space-y-3">
        {/* Live Radar Pill */}
        <div className="inline-flex items-center space-x-2 px-3.5 py-1.5 rounded-full bg-slate-900/90 border border-cyan-500/40 shadow-[0_0_20px_-3px_rgba(6,182,212,0.3)] backdrop-blur-md">
      ...
      <div
        ...
        className="relative rounded-3xl p-5 sm:p-7 bg-[#080d1a]/90 border border-slate-800/90 shadow-[0_20px_60px_-15px_rgba(6,182,212,0.15)] backdrop-blur-2xl overflow-hidden"
      >
        ...
        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-5 items-stretch">
            ...
            {topBreakouts.map((repo, idx) => {
              ...
              return (
                <button
                  ...
                  className={`w-full p-3 rounded-2xl text-left transition-all duration-200 flex items-center justify-between gap-3 border relative ${
                    isSelected
                      ? 'bg-slate-900/95 border-cyan-500/70 shadow-[0_0_20px_-3px_rgba(6,182,212,0.25)]'
                      : 'bg-slate-950/60 hover:bg-slate-900/80 border-slate-800/70 hover:border-slate-700'
                  }`}
                >
                  {/* Left Accent Bar for selected item */}
                  {isSelected && (
                    <div className="absolute left-0 top-2 bottom-2 w-1 bg-cyan-400 rounded-r-full shadow-[0_0_8px_#06b6d4]" />
                  )}
                  ...
                  {/* Stars Pill */}
                  <div className="flex-shrink-0 font-mono text-xs font-bold text-amber-300 bg-slate-950 px-2 py-0.5 rounded border border-slate-800">
          ...
          <div className="lg:col-span-7 p-5 sm:p-6 rounded-2xl bg-gradient-to-b from-slate-900/95 to-slate-950/95 border border-slate-800/90 shadow-2xl flex flex-col justify-between relative group">
            
            <div>
              {/* Card Header: Rank Badge & Velocity */}
              <div className="flex items-center justify-between gap-2 mb-3.5">
                <span
                  className={`px-3 py-1 rounded-lg text-xs font-mono font-black bg-gradient-to-r ${currentBadge.color} ${currentBadge.text} shadow-sm flex items-center gap-1.5`}
                >
                  {currentBadge.label}
                </span>

                <div className="flex items-center space-x-2 text-xs font-mono">
                  <span className="text-emerald-400 font-bold bg-emerald-950/60 px-2.5 py-0.5 rounded border border-emerald-500/30">
              ...
              <div className="flex items-start gap-3.5 mt-2">
                <img
                  ...
                  className="w-12 h-12 rounded-2xl ring-1 ring-slate-700 bg-slate-800 flex-shrink-0 shadow-md"
                  onError={(e) => {
              ...
              {/* Description */}
              <p className="text-xs sm:text-sm text-slate-300 line-clamp-3 mt-3 leading-relaxed font-normal">
                {activeRepo.description}
              </p>

              {/* Growth Delta Highlight */}
              {activeRepo.growthDeltaText && (
                <div className="mt-3.5 inline-flex items-center font-mono text-xs font-bold text-amber-300 bg-amber-950/40 px-3 py-1 rounded-lg border border-amber-500/30">
                  ⚡ {activeRepo.growthDeltaText}
                </div>
              )}
            </div>
            ...
            <div className="mt-5 pt-3.5 border-t border-slate-800 flex items-center justify-between gap-3">
              ...
              <div className="flex items-center space-x-2 flex-shrink-0">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="px-4 py-1.5 rounded-xl font-mono font-bold text-xs bg-cyan-500 hover:bg-cyan-400 text-slate-950 transition-all shadow-md flex items-center gap-1"
                >
                  <span>Explore</span>
                  <span>↗</span>
                </a>
